...
- **Pricing:** ₹5/active resident/month ("active" = bed-allocated). Manual collection (offline UPI); metered in-app via `billing_snapshots`.
...
|---|---|---|
| Tenant isolation | Shared Postgres + RLS | Cheapest at ₹5/resident economics |
...
- **Move-out request approval workflow** (2026-07-14, `deposits/`): resident move-out is now a two-tier state — `users.exitRequestedDate/exitRequestNote/exitRequestedAt` hold the manager-**approved** ("effective") move-out; `users.exitPendingType/exitPendingDate/exitPendingNote/exitPendingAt` hold a resident-initiated action (`REQUEST`/`UPDATE`/`CANCEL`) awaiting a manager decision (`POST /deposits/exit-request/{update,cancel,withdraw}` + manager `POST /deposits/exit-request/:residentId/{approve,reject}`, all conditional-flip guarded). The deposit balance itself is always computed live from `deposit_transactions`, never cached, so cancel/update never has to "restore" anything. Withdrawing your OWN undecided pending action needs no approval. `allocation.service.ts`'s `LEAVING_SOON`/bed-booking logic and the residents-list `?exitRequested=` filter read the effective (approved) columns unchanged, so they now key off approved exits only — a behavior change from before, when any raw request counted. `dashboard.service.ts`'s alerts feed was moved to key off the pending tier instead (that's the "needs a decision" signal now). Resident-web's `/deposit` page picks the move-out **month only** (stored as that month's 1st) instead of an exact day, and renders 5 states (none / pending request / approved / pending cancel / pending update); apps/mobile was intentionally left on the old single-date UI (its `exitRequest` flat fields still populate from whichever tier is live, so it keeps working unmodified — see the next bullet for why that's now a real billing gap, not just cosmetic).
- **Move-out billing: skip-and-auto-settle, + a booking guard** (2026-07-14, `rent/`, `deposits/`): once a move-out is **approved**, `RentService.generateMonthly` generates NO invoice for the exit month or any month after (string-compares each resident's billing period against `exitRequestedDate`'s period). The resident's LAST billed month (the month before the exit month) is auto-settled from their held deposit **when the deposit fully covers it** (all-or-nothing, like manual "Apply to rent" — no partial draw-down) — a 4th billing fold in `generateMonthly` for an invoice generated after approval, or an apply-now check in `approveExitRequest` for one that already existed (mirrors `ChargesService.create`'s SUBMITTED-payment guard so it never fights an in-flight payment). Both paths share `deposits/settle-invoice-from-deposit.ts` (`settleInvoiceFromDepositIfEligible` — FOR UPDATE lock + conditional PENDING/OVERDUE→PAID flip, same discipline as `applyToInvoice`). **No reversal**: pulling an exit date earlier after a later month was already auto-settled is not un-done (same class of gap as the backlogged "orphaned transfer adjustments"). **Booking guard**: cancelling or changing an approved move-out (`updateExitRequest`/`requestCancelExit`, and defensively again in `approveExitRequest`) is blocked (409) once an incoming resident's `PENDING` booking depends on the resident's current bed (`DepositsService.hasPendingBookingOnCurrentBed`, the same bed+status join `freeBed()` uses) — surfaced to both UIs as `exitRequest.bookingConflict` (resident-web hides the Change/Cancel buttons; admin disables Approve with an inline note). **apps/mobile is a real gap here, not cosmetic**: its move-out picker still takes an exact day, but the new semantics skip the resident's ENTIRE exit month regardless of day — a mobile resident meaning "I'm here until the 15th" gets that whole month unbilled. Not fixed this round.

- **Resident email verification + email notification channel** (2026-07-24, `residents/`, `mail/`, `notifications/`): interim stand-in for real push infra (deferred until 1–2 paying customers fund it). Email is now **mandatory** for long-term residents at registration (conditional Zod refine — short-stay guests exempt) and must be **verified via an emailed OTP** before a bed can be allocated or booked. `EmailVerificationService` clones the `OtpService` Redis pattern (`email_otp:{tenantId}:{residentId}`, attempt-capped) + `MailService.sendOtpEmail`; manager-driven routes `POST /residents/:id/email/verify/{request,verify}` (throttled — the throttler config is now shared via `common/throttler.config.ts`, imported by Auth + Residents modules). The gate is `assertEmailVerified()` at `AllocationService.allocate()` + `BookingsService.create()` (short-stays / transfers / the daily `activateDue` are intentionally ungated — see the guard's comment). `NotificationsService.notify()` now also **emails** the resident (best-effort, verified addresses only — a 2nd delivery channel for every feed row: payments, announcements, rent reminders) via `MailService.sendNotificationEmail`. **Recovery path**: `PATCH /residents/:id/email` corrects a mis-typed address and resets `emailVerified` (admin "Edit/Add email" affordance) — closes the "typo'd email → un-allocatable resident" dead-end. Admin residents page: required email field + verified/not-verified badge + Verify/Edit dialogs + Allocate disabled until verified. **Requires SES live in production** (`SES_FROM_EMAIL` + creds) — the console stub only logs; in dev, `OTP_DEV_LOG=true` prints the code. `email-verification.e2e-spec.ts` covers the gate + exemption + recovery + cross-tenant. **apps/mobile / resident-web unchanged** (verification is a manager action).
