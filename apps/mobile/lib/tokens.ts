 ...
 *
 * Three consumers:
 *   1. `themeVars(scheme, accent)` → the CSS-variable set ThemeProvider applies
 *      via NativeWind `vars()` (and Sheet re-applies on Modal roots — RN Modals
 *      portal outside the root tree, so vars don't cascade into them).
 *   2. `resolveTokens(scheme, accent)` → a flat JS object for everything that
 *      needs a real color value: Ionicons `color`, placeholderTextColor,
 *      RefreshControl tints, StatusBar, the bottom tab bar.
 *   3. tailwind.config.js maps class names (`text-ink`, `bg-amber-bg`, …) to
 *      these vars; global.css seeds the light defaults pre-provider.
 *
