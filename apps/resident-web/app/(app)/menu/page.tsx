...
export default function MenuPage() {
  ...
  const to = ymd(days[6]);
  const { data, isLoading, isError, refetch } = useMenu(from, to);
  ...
  return (
    <Screen contentClassName="gap-3">
      ...
      <div className="flex flex-row items-center justify-between rounded-pill bg-brand-soft px-1.5 py-1">
        <PressableScale
          onClick={() => setWeek((w) => w - 1)}
          haptic="selection"
          pressedScale={0.85}
          aria-label="Previous week"
          className="flex h-10 w-10 items-center justify-center rounded-full text-brand-deep"
        >
