 ...
 * stay screen-local (they invalidate these keys), keeping cache invalidation
 * consistent across screens (e.g. submitting a payment invalidates `invoices`).
 * Ported 1:1 from the mobile app (apps/mobile/lib/queries.ts).
 */
export const qk = {
...
// while the thread screen is mounted and pauses while the tab is hidden
// (refetchIntervalInBackground defaults false).
